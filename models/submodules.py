...
class BuildVolume2d(nn.Module):
    ...
    def forward(self, feat_l, feat_r):
        # print('BuildVolume2d: forward start')
        # print(feat_l.size()) # torch.Size([1, 32, 17, 12])
        # print(feat_r.size()) # torch.Size([1, 32, 17, 50])

        gc.collect()
        torch.cuda.empty_cache()
        
        padded_feat_r = F.pad(feat_r, [self.maxdisp-1, 0, 0, 0])
        # print(padded_feat_r.size())
        cost = torch.zeros((feat_l.size()[0], self.maxdisp, feat_l.size()[2], feat_l.size()[3]), device='cuda')
        # print(feat_l[:, :, :, :].size()) # torch.Size([1, 32, 17, 12])
        # print(padded_feat_r[:, :, :, self.maxdisp-1::4].size()) # torch.Size([1, 32, 17, 13])
