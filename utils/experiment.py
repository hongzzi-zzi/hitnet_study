...
def make_iterative_func(func):
    def wrapper(vars):
        if isinstance(vars, list):
            ...
            newdict={k:wrapper(v) for k, v in vars.items()}
            return  newdict
...
def save_images(logger, mode_tag, images_dict, global_step):
    ...
    # print(type(images_dict))# dict
    
    # convert image_outputs type(dict->numpy)
    # print(isinstance(vars,dict))
    # images_dict=list(images_dict.items())
    
    # print(type(images_dict))# dict
    # print(isinstance(vars,list))

    # images_dict=np.array(images_dict)
    # torch.stack([images_dict],dim=0)
    
    # images_dict.numpy()
    # images_dict.tolist()
    
    # images_dict = tensor2float(images_dict)
    # print('images_dict shape: {}'.format(images_dict.shape))
    
    '''왜 images_dict = tensor2float(images_dict) 전후 둘다 dict인데 하는걸가..?'''
    ...
    for tag, values in images_dict.items():
        ...
        for idx, value in enumerate(values):
            ...
            value = value[:1]
            # value=cpu()
            # value= Tensor.detach(value).numpy().array(value).astype('float')
